...
.EXAMPLE  
    ...
    60 minutees (1 hour) to send logs to Azure Storage Account
...
.DESCRIPTION 
 Using an Azure Automation Account Runbook, upload logs to Azure Storage Account. 
...
function Send-LogsToAzureStorageAccount () {
    Param(
    ...
    try {
        ...
        stream of JSON data is stored in the Azure Storage Account. Please rerun the script `
    ...
    try {
        Write-Output "Streaming/Uploading the results to the Azure Storage Account."
        $AzureStorageAccountBlobSasUrl = $AzureStorageAccountBlobSasUrl.replace("?","/$($key)?")
        ...
        Invoke-RestMethod -Uri $AzureStorageAccountBlobSasUrl -Method Put -Headers $headers -Body $Global:jsonResults  -ContentType application/json
    }
    catch {
        ...
        the AzureStorageAccountBlobSasUrl exists and is correct, trouble connectivity to the Azure 
        Storage Account manually, or contact cloud administrator. This is a fatal error `
...
try {
    $AzureStorageAccountBlobSasUrl = Get-AutomationVariable -Name AzureStorageAccountBlobSasUrl
}
catch {
    Write-Error "Failed to collect Azure Storage Account Blob SAS URL, please verify variable exists `
    in the same Automation Account in which this script was run."
}
