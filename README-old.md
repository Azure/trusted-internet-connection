...
Architecture solutions are defined by two categories, Azure Firewall and third-party firewalls. Azure Firewall is natively configured to send logs to a Log Analytics workspace while third-party firewalls typically send logs to a Log Analytics workspace using the vendors' syslog export feature. 
...
Azure Firewall is natively supported by the deployment scenarios. Third-party firewall, aka Network Virtual Appliance (NVA), require a syslog forward to be available to receive the export of the NVA and then forward the logs to the Log Analytics workspace. 
